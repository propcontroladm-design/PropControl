  ...
  return(
    <div style={S.modal} onClick={(e:any)=>{if(e.target===e.currentTarget)onClose()}}>
      <div style={S.modalBox}>
        ...
        <div style={{background:'#F0FDF4',padding:11,borderRadius:10,marginBottom:11,border:'1px solid #BBF7D0'}}>
          <label style={{...S.lbl,marginBottom:5}}>📊 Empezar a controlar pagos desde</label>
          <input style={S.inp} type="date" value={d.fecha_control_desde||''} onChange={e=>up('fecha_control_desde',e.target.value)} placeholder="Opcional"/>
          <div style={{fontSize:11,color:'#15803D',marginTop:6,lineHeight:1.4}}>
            💡 Si el contrato empezó hace tiempo pero recién empezás a usar la app, poné acá <strong>desde qué mes</strong> querés que se controlen los pagos. Los meses anteriores se asumirán como ya cobrados y no aparecerán como deuda.
          </div>
        </div>
...
export default function Dashboard(){
  ...
  const renderInicio=()=>{
    ...
    const mesVisto={year:mesVistoYear,month:mesVistoMonth,key:mk(mesVistoYear,mesVistoMonth)}
    const esMesActual=mesVisto.key===mesActK
    ...
    // Filtrar contratos activos en el mes visto (que ya hayan iniciado)
    const inicioMesVisto=new Date(mesVisto.year,mesVisto.month,1).getTime()
    const finMesVisto=new Date(mesVisto.year,mesVisto.month+1,0,23,59,59).getTime()
    const rows=contratos.filter(c=>{
      if(c.activo===false)return false
      // No incluir contratos que aún no comenzaron en el mes visto
      if(c.fecha_inicio){
        const ini=new Date(c.fecha_inicio+'T00:00:00').getTime()
        if(ini>finMesVisto)return false
      }
      // No incluir contratos que ya terminaron antes del mes visto
      if(c.fecha_fin){
        const fin=new Date(c.fecha_fin+'T23:59:59').getTime()
        if(fin<inicioMesVisto)return false
      }
      // NUEVO: si fecha_control_desde es posterior al mes visto, no incluir (no se controla todavía)
      if(c.fecha_control_desde){
        const ctrl=new Date(c.fecha_control_desde+'T00:00:00').getTime()
        if(ctrl>finMesVisto)return false
      }
      return true
    }).map(c=>{
      ...
      const estado=espP===0&&total===0?'pendiente':(espP>0&&diff>=-0.5?'pagado':(total>0?'parcial':'pendiente'))
      const inq=inqs.find(i=>i.id===c.inquilino_id)
      // Detectar deuda histórica respetando "fecha_control_desde"
      let mesesDebidos=0
      // Punto de partida: el más reciente entre fecha_inicio y fecha_control_desde
      let fechaInicioControl:Date|null=null
      if(c.fecha_control_desde){
        fechaInicioControl=new Date(c.fecha_control_desde+'T00:00:00')
      }else if(c.fecha_inicio){
        fechaInicioControl=new Date(c.fecha_inicio+'T00:00:00')
      }
      if(fechaInicioControl){
        let y=fechaInicioControl.getFullYear(),m=fechaInicioControl.getMonth()
      ...
      return{c,mObj,lista,total,diff,estado,espP,inq,mesesDebidos}
    })
    ...
    function mesAnterior(){
      let m=mesVistoMonth-1,y=mesVistoYear
      if(m<0){m=11;y--}
      setMesVistoMonth(m);setMesVistoYear(y)
    }
    ...
    function irHoy(){
      setMesVistoMonth(NOW.getMonth());setMesVistoYear(NOW.getFullYear())
    }
    
    return(
      <div style={{padding:14}}>
        {/* Selector de mes */}
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:11,background:'white',borderRadius:11,padding:'8px 10px',border:'1px solid #E2E8F0',boxShadow:'0 1px 2px rgba(0,0,0,0.04)'}}>
          <button onClick={mesAnterior} style={{background:'#F1F5F9',color:'#475569',border:'none',width:32,height:32,borderRadius:8,cursor:'pointer',fontSize:16,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center'}}>‹</button>
          <div style={{flex:1,textAlign:'center'}}>
            <div style={{fontSize:10,color:'#94A3B8',fontWeight:600,letterSpacing:0.5,textTransform:'uppercase'}}>{esMesActual?'Mes actual':(inicioMesVisto<Date.now()?'Mes pasado':'Mes futuro')}</div>
            <div style={{fontSize:15,fontWeight:700,color:'#0F172A',textTransform:'capitalize'}}>{mlbl(mesVisto.year,mesVisto.month)}</div>
          </div>
          <button onClick={mesSiguiente} style={{background:'#F1F5F9',color:'#475569',border:'none',width:32,height:32,borderRadius:8,cursor:'pointer',fontSize:16,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center'}}>›</button>
          {!esMesActual&&<button onClick={irHoy} style={{background:'#22C55E',color:'white',border:'none',padding:'7px 11px',borderRadius:8,cursor:'pointer',fontSize:11,fontWeight:700}}>Hoy</button>}
        </div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:9}}>
          <div><p style={{fontSize:11,color:'#6b7280',margin:0}}>Resumen del mes</p></div>
          {rows.length>0&&<div style={{textAlign:'right'}}><p style={{fontSize:10,color:'#6b7280',margin:0}}>Cobrado / Esperado</p><p style={{fontSize:13,fontWeight:700,margin:0}}>{fmtN(totCob,'pesos')} / {fmtN(totEsp,'pesos')}</p></div>}
        </div>
            ...
            <div key={c.id} style={{background:'white',borderRadius:14,border:'1px solid #e5e7eb',padding:13,marginBottom:9,boxShadow:'0 1px 3px rgba(0,0,0,.07)'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:4}}>
                <div style={{flex:1,minWidth:0}}>
                  ...
                  {mesesDebidos>0&&esMesActual&&<div style={{display:'inline-block',marginTop:4,background:'#FEE2E2',color:'#991B1B',padding:'2px 8px',borderRadius:6,fontSize:11,fontWeight:700}}>⚠️ {mesesDebidos} mes{mesesDebidos>1?'es':''} adeudado{mesesDebidos>1?'s':''}</div>}
                </div>
                <Badge e={estado}/>
              </div>
              ...
              <div style={{display:'flex',gap:5}}>
                <button style={{flex:1,background:'#2563eb',color:'white',padding:9,borderRadius:9,fontWeight:700,fontSize:13,cursor:'pointer',border:'none'}}
                  onClick={()=>setModal({type:'pago',contrato:c,mes:mesVisto,mObj,lista})}>
